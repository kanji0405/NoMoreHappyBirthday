...
/*:
 * @plugindesc 【v2.16】戦闘システムをアクティブタイムバトルに変更します。
 * @author 木星ペンギン
 *
 * @help ▼ システム説明
 * --------------------------------
...

{
    ...
    const parameters = PluginManager.parameters('MPP_ActiveTimeBattle');
    ...
    const se = JSON.parse(parameters['Active SE _v2']);
    MPPlugin.ActiveSE = {
        name:se.Name || "",
...
MppOptions.getStatus = function(symbol) {
    var param = this.params.find(param => param.symbol === symbol);
    return param ? param.status : [];
};
...
for (const param of MppOptions.params) {
...
ConfigManager.makeData = function() {
    ...
    for (const { symbol } of MppOptions.params) {
...
ConfigManager.applyData = function(config) {
    ...
    for (const { symbol } of MppOptions.params) {
        if (typeof config[symbol] === 'number') this[symbol] = config[symbol];
    }
};
...
Window_Options.prototype.makeCommandList = function() {
    for (const param of MppOptions.params) {
        if (param.name) this.addCommand(param.name, param.symbol);
    }
...
BattleManager.updateATB = function() {
    ...
    if (Graphics.frameCount % 2 === 0) {
        ...
        for (const battler of allBattlers) {
            if (battler.isMadeAction()) this.addActionBattler(battler);
        }
    }
};
...
BattleManager.updateCmdActor = function() {
    if (this.isEventWait()) {
        ...
        for (const [i, actor] of $gameParty.battleMembers().entries()) {
            if (actor.isStandby()) return this.changeActor(i);
        }
...
BattleManager.isEventWait = function() {
    return $gameTroop.isEventRunning() || $gameMessage.isBusy();
};

BattleManager.isAtbWait = function() {
    return (MPPlugin.StopTimeOnAction && this._subject) ||
            (this.isEventWait() || this._waitHandler());
};
...
//
Alias.GaAc_onRestrict = __base(Game_Actor.prototype, 'onRestrict');
Game_Actor.prototype.onRestrict = function() {
    Alias.GaAc_onRestrict.apply(this, arguments);
...
Game_Unit.prototype.selectAll = function() {
    for (const battler of this.members()) {
        battler.select();
    }
};
...
Game_Unit.prototype.select = function(activeMember) {
    for (const battler of this.members()) {
        battler === activeMember ? battler.select() : battler.deselect();
    }
};
...
Game_Party.prototype.addActor = function(actorId) {
    ...
    if (this.inBattle()) BattleManager.refreshStatus();
};
...
Alias.WiBaAc_hitTest = __base(Window_BattleActor.prototype, 'hitTest');
Window_BattleActor.prototype.hitTest = function(x, y) {
    var result = Alias.WiBaAc_hitTest.apply(this, arguments);
...
Window_BattleEnemy.prototype.selectForItem = function() {
    ...
    var action = actor.inputtingAction();
    this.setCursorAll(false);
    if (action.isForAll()) this.setCursorAll(true);
    this.select(0);
};

Alias.WiBaEn_hitTest = __base(Window_BattleEnemy.prototype, 'hitTest');
Window_BattleEnemy.prototype.hitTest = function(x, y) {
    var result = Alias.WiBaEn_hitTest.apply(this, arguments);
    if (result >= 0 && this._cursorAll && this.isContentsArea(x, y)) {
        return 0;
    }
    return result;
...
Window_BattleEnemy.prototype.refresh = function() {
    Alias.WiBaEn_refresh.apply(this, arguments);
    if (this.index() >= 0) {
...
window.Window_AtbSkillStatus = Window_AtbSkillStatus;
